...
//        }
        ...
        let userInfo = firestoreUserInfo.document(userUID).getDocument { documentSnapshot, error in
            ...
            do {
                //let data = try! JSONSerialization.data(withJSONObject: document.data() as? NSDictionary)
                let t = try document.data(as:UserInfoViewModel.self)
                print(t)
//                let t = try JSONDecoder().decode(UserInfoViewModel.self, from: data)
            } catch let e{
