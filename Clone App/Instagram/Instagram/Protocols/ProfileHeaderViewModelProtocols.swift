...
/**
    이 헤더의 특징은 input은 없다. ProfileController에 의해 viewModel에서 사용자의 정보, 프로필, 팔로우 개수를 받아오면
    viewModel의 inner publisher properties의 값이 갱신된다. 이 점을 이용해서 ProfileHeaderViewModelOutput을 통해 ProfileHeader
    의 뷰를 갱신한다.
    input은 그저 viewModel _user, _profileImage, _userStats에 각각의 값이 받아질 때마다
