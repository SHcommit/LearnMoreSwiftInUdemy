...
extension MainHomeTabViewModel {
  ...
  func fetchUserInfoFromUserService() async throws {
    guard let user = try await apiClient.userCase.fetchCurrentUserInfo(type: UserModel.self) else { throw FetchUserError.invalidUserInfo }
    self.user = user
  }
